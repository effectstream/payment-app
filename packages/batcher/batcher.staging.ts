...
if (!BATCHER_EVM_SECRET_KEY)
  throw new Error("BATCHER_EVM_SECRET_KEY is required for staging batcher");
if (!EFFECTSTREAM_L2_ADDRESS)
  throw new Error("EFFECTSTREAM_L2_ADDRESS is required for staging batcher");
...
main(function* () {
  console.log("Starting Payment App batcher (staging — Ethereum Sepolia)...");
  try {
